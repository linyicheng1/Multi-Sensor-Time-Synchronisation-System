...
准备**全功能时间同步板**或**时间同步核心板**，核心板尺寸较小适合无人机搭载，**全功能时间同步板**与树莓派尺寸相同，安装孔位对应。连接好需要同步的传感器和电源。硬件接口的定义如下：
...
在firmware文件夹下，有mini_xx.uf2文件和full_xx.uf2文件，分别对应最小功能固件和全功能固件。其中full表示全功能，V3表示第三代同步板。将对应uf2文件下载，然后将板子连接通过Type-C连接到电脑上，按住板子上的BOOT按键，然后按一下RESET按键，然后松开BOOT按键，这时候板子会进入BOOT模式，弹出一个虚拟U盘。
然后将uf2文件拖到板子的U盘中，等待固件烧入完成。 烧录完成后，你可以参考[全功能同步板快速开始](./assets/全功能同步板快速开始.md)与[核心板快速开始](./assets/核心板快速开始.md)进行快速使用
